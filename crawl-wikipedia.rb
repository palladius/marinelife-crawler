...
def smart_morgan_freeman(actor = "Morgan_Freeman")
  ...
  html = Nokogiri::HTML(URI.open("https://en.wikipedia.org/wiki/" + actor.gsub(' ','_')))
  ...
  filmo_list.at_css('i').each { |l| puts l } rescue nil
end
...
def fish_page_get_info(path, opts={})
  ...
  h = {
    ...
    :fish => {},
    :autofish => {}, # automatically dump to YAML without asking with alreayd proper stuff :)
  }
...
#  h[:looks_like_an_animal] = online_page.to_s.match?('<a href="/wiki/Animal" title="Animal">Animalia</a>')
  h[:looks_like_an_animal] = magic_parsed_page.to_s.match?('<a href="/wiki/Animal" title="Animal">Animalia</a>').to_s == 'true'
...
#  h[:prova_title] = online_page.css(title_selector).to_s
  ...
  if ($DEBUG)
    ...
    h[:a_links].each do |tag|
      puts "[LINK_FIGO] #{tag[:href]}\t#{tag.text}"
    end if opts_debug_links
  ...
  if h[:looks_like_an_animal]
    ...
    simple_description_css_selector = '#mw-content-text > div.mw-parser-output > p:nth-child(4)'
    ...
    wiki_page_skeleton = magic_parsed_page.css('#mw-content-text > div.mw-parser-output > *')
    ...
    h[:fish][:simple_description] = magic_parsed_page.at_css(simple_description_css_selector).text rescue ''
    # 'TODO find right css :)'
...
def parse_fish_taxonomy(document)
  ...
  taxo_rows.each_with_index do |row, ix|
    ...
    if taxo_match = cleaned_up_text.match(/^[ ]+(Kingdom|Subkingdom|Phylum|Class|Family|Clade|Order|Index|Genus|Species|Superphylum):( )+([\w ]+)$/)
      ...
      raise "FATAL some CRLFs in name! #{my_instance}" if my_instance.match? /\n/
      #taxo_hash[my_class.to_s + '2'] = cleaned_up_text.split(':')[1].trim # chomp
      #puts "FUNGE! Vedi: #{row}"
      #puts "  - GOTCHA[#{ix}] '#{cleaned_up_text}' " # [#{row}]\n\n" # if $DEBUG
    elsif  taxo_match_ripescaggio = cleaned_up_text.match(/^[ ]+(Kingdom|Phylum|Class|Order|Family|Genus|Species):( )+([\w ]+)$/)
  ...
  taxo_hash[:carlessian_taxonomy_with_index] = arr_carlessian_taxonomy_with_index.join(' > ')
  ...
  taxo_image_css_selector = '#mw-content-text > div.mw-parser-output > table > tbody > tr:nth-child(2) > td > a > img'
  noko_imagez = document.css(taxo_image_css_selector)
  noko_imagez.each_with_index do |image, ix|
    image_url = 'https:' + image['src'] rescue nil # .css('.src')
...
def dump_fish_info_into_yaml_file(fish_hash, filename, is_it_first_today=false)
  # initialize if needed
  puts "dumping fish info into this file '#{filename}'. if first time (#{is_it_first_today}) im gonna alos delete it first"
  File.write(filename, "# initialized on #{Time.now} from #{$0} v#{Version}\n\n") if is_it_first_today
  ...
  useful_but_commented_debug_info = fish_hash[:autofish][:taxo_removeme_debug].to_yaml.gsub(/^/, "#DEBI# ")
  File.open(filename, 'a') { |f|
    f.write("#"* 80 + "\n")
...
def main()
  ...
  # END / finally
  puts "☢️⚠️⛔️ ACTHUNG! Note that ONLINE has been disabled. Lot of juicy stuff wont work, probably cos Riccardo is on a plane" unless online?
...
iterate_through_files_in_directory('samples/')
#iterate_through_files_in_directory('en.wikipedia.org/wiki')
#iterate_through_files_in_directory('en.wikipedia.org/wiki')
